...
Inductive function_bijective {A B : Type} (f: A -> B): Prop :=
  | fun_bij : function_injective f -> function_surjective f -> function_bijective f.
...
Definition injection_trans {A B C : Type} (f1: injection A B) (f2: injection B C): injection A C.
  apply (Build_injection A C (fun (x : A) => f2 (f1 x))). unfold function_injective. intros.
  pose proof (in_inj A B f1). unfold function_injective in H0.
  pose proof (in_inj B C f2). unfold function_injective in H1.
  apply H1 in H. apply H0 in H. apply H.
Defined.

Definition bijection_injection {A B : Type} (f : bijection A B) : injection A B.
  apply (Build_injection A B f). unfold function_injective. intros.
  pose proof (in_bij A B f). unfold function_injective in H0. apply H0.
  apply H.
Defined.

Definition bijection_surjection {A B : Type} (f : bijection A B) : surjection A B.
  apply (Build_surjection A B f). unfold function_surjective. intros.
  pose proof (su_bij A B f). unfold function_surjective in H. apply H.
Defined.
