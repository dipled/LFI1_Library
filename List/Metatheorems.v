...
Lemma quasi_monotonicity : forall (Γ : list Formula) (α β : Formula), (Γ ⊢ β) -> (α :: Γ) ⊢ β.
Proof.
  intros. induction H.
...
Theorem deduction_metatheorem : forall (Γ : list Formula) (α β : Formula), ((α :: Γ) ⊢ β) <-> (Γ ⊢ α → β).
Proof. 
  intros. split.
...
Corollary proof_by_cases : forall (Γ : list Formula) (α β φ : Formula), ((α :: Γ) ⊢ φ) /\ (β :: Γ ⊢ φ) -> ((α ∨ β :: Γ) ⊢ φ).
Proof.
  intros. destruct H. 
...
Corollary proof_by_cases_neg : forall (Γ : list Formula) (α φ : Formula), ((α :: Γ) ⊢ φ) /\ ((¬ α :: Γ) ⊢ φ) -> (Γ ⊢ φ).
Proof.
  intros. destruct H.
  pose proof deduction_metatheorem as DMT.
  apply DMT in H. apply DMT in H0.
